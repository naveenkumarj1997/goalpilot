...
const WisdomReader = () => {
  ...
  if (loading) return <div className="flex justify-center items-center h-full min-h-[500px]"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-amber-500"></div></div>;
    ...
    <div className="max-w-4xl mx-auto pb-12 animate-slide-up-fade relative">
      ...
      <div className="glass rounded-3xl overflow-hidden border border-slate-700/50 flex flex-col md:flex-row min-h-[600px] relative">
        ...
        <div className="w-full md:w-1/3 bg-slate-900/50 p-6 border-r border-slate-700/50 flex flex-col items-center">
          ...
          <div className="w-full space-y-2 flex-1 overflow-y-auto custom-scrollbar pr-2">
            <button 
        ...
        <div className="w-full md:w-2/3 bg-white/5 p-8 relative flex flex-col">
            ...
            {currentPage === 0 ? (
              ...
              <div className="animate-fade-in">
                {book.lessons.filter((l: any) => l.lessonNumber === currentPage).map((lesson: any) => (
                  <div key={lesson.lessonNumber} className="space-y-8">
                    ...
                    <div className="bg-emerald-900/20 p-5 rounded-2xl border border-emerald-500/30 relative">
                      ...
                      <button 
                        ...
                        className="absolute bottom-4 right-4 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center shadow-lg transition-transform hover:scale-105"
                      >
                        <Target className="w-4 h-4 mr-2" /> Push to Mission Control
                      </button>
                    </div>
            ...
            <div className="flex gap-4">
              {currentPage > 0 && !isLearned && (
                <button 
                  ...
                  className="px-6 py-2 bg-emerald-600/20 text-emerald-400 border border-emerald-500/50 rounded-xl font-bold flex items-center hover:bg-emerald-600/30 transition-colors"
                >
                  <CheckCircle className="w-5 h-5 mr-2" /> Mark as Learned
                </button>
              )}
              {currentPage > 0 && isLearned && (
                <div className="px-6 py-2 bg-slate-800 text-emerald-500 rounded-xl font-bold flex items-center">
                  <CheckCircle className="w-5 h-5 mr-2" /> Learned
                </div>
              )}
              
              <button 
                onClick={() => setShowCoach(true)}
                className="px-6 py-2 bg-brand text-white rounded-xl font-bold flex items-center shadow-[0_0_15px_rgba(0,112,209,0.4)] hover:bg-brand-hover transition-colors"
              >
                <MessageSquare className="w-5 h-5 mr-2" /> Ask AI Coach
              </button>
            </div>
