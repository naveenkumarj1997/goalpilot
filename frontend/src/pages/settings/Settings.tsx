...
export default function Settings() {
  ...
  const [dreamCheckInTime, setDreamCheckInTime] = useState('20:00');
  ...
  useEffect(() => {
    ...
    if (userStr) {
      ...
      if (user.dreamCheckInTime) {
        setDreamCheckInTime(user.dreamCheckInTime);
      }
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedUser = await authService.updateSettings({ dailyCheckInTime, nofapCheckInTime, dreamCheckInTime });
      if (updatedUser) {
    ...
    <div className="max-w-2xl mx-auto">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 relative z-10">
          ...
          <div>
            <h2 className="text-lg font-semibold text-text-primary mb-4">Dream Savings Check-In</h2>
            <p className="text-sm text-text-secondary mb-4">
              Choose the time of day you would like to be prompted to log your daily dream & bucket list savings.
            </p>
            
            <div className="max-w-xs">
              <label htmlFor="dreamCheckInTime" className="block text-sm font-medium text-text-primary mb-1">
                Check-In Time
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <input
                  type="time"
                  id="dreamCheckInTime"
                  value={dreamCheckInTime}
                  onChange={(e) => setDreamCheckInTime(e.target.value)}
                  className="w-full pl-10 px-4 py-2 border border-emerald-200 rounded-xl focus:ring-2 focus:ring-brand focus:border-brand transition-shadow bg-white/50"
                  required
                />
              </div>
