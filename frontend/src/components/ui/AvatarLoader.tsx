...
export default function AvatarLoader({ size = 'xl', className = '' }: AvatarLoaderProps) {
  const sizeClasses = {
    sm: 'w-6 h-6 border-2',
  ...
  return (
    <div className={`flex items-center justify-center min-h-[400px] w-full ${className}`}>
      <div 
        className={`animate-spin rounded-full border-transparent border-t-[#D4AF37] border-r-[#D4AF37] ${sizeClasses[size || 'xl']}`}
        style={{
